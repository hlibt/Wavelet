...
int main(void) {
    ...
    double dt=(t_f-t_i)/steps;                                  // timestep size
    ...
    double** x=new double*[J];                                  // collocation points
    for (j=0;j<J;j++) x[j]=new double[N];                       //
    for (j=0;j<J;j++) {                                         //
        for (k=0;k<=pow(2,j);k++) {                             //
    ...
    //------- STEP 4: APPROXIMATE INITIAL CONDITION ------------//
...
void time_stamp(int time,double diff,double dt) {
    ...
    //------- STEP 5: ADJUST THE DYADIC GRID, Gt+1 -------------//    
    // adjust Gt+1 based on coefficients and epsilon parameter
    //------- STEP 6: IF Gt != Gt+1, EVALUATE U AT NEW POINTS---//
    //------- STEP 7: INTEGRATE SOLUTION IN TIME ---------------//
    //double* U_new=new double[2*M];                              // initialize solution matrix U        
    //------- STEP 8: OUTPUT SOLUTION TO FILE ------------------//    
    ofstream output;                            //                                      //
    char fn[20];                                //                                      //
    snprintf(fn,sizeof fn,"../output/%04d.dat",s); //                                      //
    output.open(fn);                            //                                      //
