...
double* complete_pivot(double** A,double* b,double tol,int size,int mxi);
...
int main(void) {
    ...
    int M=4;                                        // half of collocation points           //
    int mxi=2000;                                    // max no. of iterations 4 matrix solvr //
...
bool chk_conv(double** A,double* y,double* b,double tolerance,int n) {
    ...
    bool conv;
    tmp=ADOTX(A,y,n);
    for (int i=0;i<n;i++) residual[i]=tmp[i]-b[i];
